...
@section('content')
    ...
    <section class="py-4">
        <div class="max-w-[30rem] mx-auto">
            <div class="">
            <p>({{ collect(Session::get('cart'))->sum('qty_panier') }}) produits ajouté(s) au <a class="
                italic underline" href="{{ route('cart.index') }}">Panier</a></p>
                <p>Total à payer: $ <span
                        class="font-bold">{{ array_reduce(Session::get('cart'), fn($c, $i) => $c + $i['prix'] * $i['qty_panier']) }}</span>
                </p>
            </div>
            <h1 class="mt-4 text-4xl font-bold">Livraison</h1>
            <form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post" x-data="order" id="formOrder"
                x-ref="formOrder">
                ...
                <input type="hidden" name="item_name" value="({{ collect(Session::get('cart'))->sum('qty_panier') }})">
                ...
                <input type="hidden" name="email" value="sb-9lwfe7657189@personal.example.com">
                ...
                <div class="mt-6 sm:max-w-xs sm:mx-auto">
                    <button class="btn btn-primary" name="submit">Confirmer et payer </button>
                    <p class="mt-4 text-sm">Note: Pour le moment, on ne supporte que Paypal</p>
                </div>
